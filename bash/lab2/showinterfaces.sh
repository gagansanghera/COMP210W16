...
#   displays their configured IPV4 addresses as well as default route information Get an array of our interface names, we will have at least 2
interfaces=(`ifconfig |grep '^[A-Za-z]'|awk '{print $1}'`)
# this was for debugging purposes echo "Our interfaces include ${interfaces[@]}" For the first interface in the array, get the IP address and
